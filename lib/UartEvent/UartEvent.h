...
#define RX0_BUFFER_SIZE 256  // Uart1 incoming buffer size, must be power of 2 //
                                                                              //
#define TX1_BUFFER_SIZE 128  // Uart2 outgoing buffer size, must be power of 2 //
#define RX1_BUFFER_SIZE 128  // Uart2 incoming buffer size, must be power of 2 //
                                                                              //
...
private:
    ...
    void serial_dma_set_tx_pin       ( uint8_t pin );
    void serial_dma_set_rx_pin  ( uint8_t pin );
    ...
    void serial_clear_rx_event       ( void );
    void serial_clear_tx_event       ( void );
...
public:
    ...
    virtual void   set_transmit_pin ( uint8_t pin )     { serial_dma_set_tx_pin( pin ); }
    virtual void   set_recieive_pin ( uint8_t pin )     { serial_dma_set_rx_pin( pin ); }
    ...
    virtual void clear_rx_event     ( void )            { serial_clear_rx_event( ); }
    virtual void clear_tx_event     ( void )            { serial_clear_tx_event( ); }
...
public:
    ...
    virtual void   clear            ( void )            { serial_dma_clear( ); }
    virtual int    availableForWrite( void )            { return serial_dma_write_buffer_free( ); }
    virtual void   set_transmit_pin ( uint8_t pin, uint8_t opendrain )     { serial_dma_set_tx_pin( pin, opendrain = 0); }
    virtual void   set_recieve_pin  ( uint8_t pin )     { serial_dma_set_rx_pin( pin ); }
    virtual size_t write            ( uint8_t c )       { serial_dma_putchar( c ); return 1; }
    virtual size_t write            ( unsigned long n ) { return write( ( uint8_t )n ); }
    virtual size_t write            ( long n )          { return write( ( uint8_t )n ); }
    virtual size_t write            ( unsigned int n )  { return write( ( uint8_t )n ); }
