...
#End Region
    Private Sub showstart_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
            com8.Dispose()
    ...
    Private Sub read()
        Try
            ...
            Try
                Do  'check data from arduino until process is complete
                    ...
                    If upload = "sensors" Then  'arduino has been restarted
                        ...
                        Dim rs4 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs5 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs6 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs7 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs8 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs9 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs10 = CInt(com8.ReadLine)  'add extra samples here
                        Dim rs11 = CInt(com8.ReadLine)  'add extra samples here
                        ...
                        Do   'upload all ram
                            ...
                            If upload.Substring(0, upload.Length - 1) = "r" Then
                                date_stamp = date_stamp.Subtract(TimeSpan.FromSeconds(ram_number * t_sam / n_sam))
                        ...
                        Dim j = lastblock
                        date_stamp = date_stamp.Subtract(TimeSpan.FromSeconds((currentblock - lastblock - 1) * n_sec * t_sam))
                        ...
                        While Not j = currentblock  'upload all sectores
                            ...
                            date_stamp = date_stamp.Add(TimeSpan.FromSeconds(n_sec * t_sam))
                        ...
                        Dim missed_samples = CInt((DateTime.Now - date_stamp_start).TotalSeconds / t_sam)
                        If missed_samples > 0 Then  'we have missed samples during upload so fill in
                            ...
                            For i = 0 To missed_samples  'use the lst sample in ram
                                ...
                                bwrf.Write(rs4)  'send the sample to disk
                                bwrf.Write(rs5)  'send the sample to disk
                                bwrf.Write(rs6)  'send the sample to disk
                                bwrf.Write(rs7)  'send the sample to disk
                                bwrf.Write(rs8)  'send the sample to disk
                                bwrf.Write(rs9)  'send the sample to disk
                                bwrf.Write(rs9)  'send the sample to disk
                                bwrf.Write(rs10)  'send the sample to disk
                            Next     'add extra samples here
                        ...
                        If button_clicked Then
                            ...
                            If currentblock - lastblock > 0 Then  'oldsectors to upload
                                ...
                                If My.Computer.FileSystem.FileExists(cd & "\" & lastblock - 1) Then
                                    ...
                                    Next
                                    date_stamp = CDate(br.ReadString) 'get the time from the file -then subtract number that are there to upload
                                    date_stamp = date_stamp.Add(TimeSpan.FromSeconds(n_sec * t_sam * (currentblock - lastblock))) 'from the beginning of the sectors
            ...
            End Try
        Catch ex As Exception
            If ex.Message = "Access to the port 'com4' is denied." _
            Or ex.Message = "The port 'com4' does not exist." _
